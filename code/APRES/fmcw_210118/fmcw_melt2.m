function site = fmcw_melt2(filename1,filename2,cfgi,fBurst,gBurst)
...
%cfg = fmcw_process_config_vsr; % Load default configuration HJH 25/05/20
% Shifted to cfg generation to batch processing 
...
if cfg.doPlotFit || cfg.doPlotAll
   ...
   % erbar(range,dh,dhe,-dhe,'k','k'); % errors (HJH commented 20/5/20)
    ...
    switch cfg.bedShiftMethod
        case 'rangeDiff'
            ...
            if bed.fractionWrapped < cfg.bed.maxWrapFraction
                ...
                bed.dhe = max([bed.dhsPhasnseGrad bed.dheEmperical]);
            else
