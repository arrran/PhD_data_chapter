...
@author: whitefar
"""
##this is an update on process_radar, trying to export every imported trace. ie no stacking at all.
...
class radarsurvey:
    ...
    def load_radar_data(self,path = "/Volumes/arc_04/FIELD_DATA/K8621920/RES/"):
            ...
            self.radata = pd.DataFrame({'datetime':datetime_raw})
    ...
    def stack_each_timestamp(self):
            """
            This stacks temporaly so that there is only one trace per timestamp
            """
            ...
            self.ch0_raw = self.ch0
            del self.ch0            
            
            stacked_ch0 = np.zeros([len(splittimes_index)-1,self.ch0_raw.shape[1]])
